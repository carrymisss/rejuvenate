...
import { LogOut, Search as SearchIcon, Plus, Book, Settings } from 'react-feather';//Bookmark
import { ArrowReturnLeft } from 'react-bootstrap-icons';
import { Row, Col, Button, Space, Input, Menu, Dropdown, Affix, message, Badge, AutoComplete, Avatar, Spin, Tag } from 'antd';
...
const HeaderIsRegister = ({ currentUserName, currentUserAvatar, isVerify, setInitialData, setPreload }) => {
   ...
   const dropdownMenu = (
      <Menu>
         <Menu.Item key="0">
            <Link to={`/${currentUserName}/create`}>{<Plus size="16" />}&nbsp;Створити</Link>
         </Menu.Item>
         <Menu.Item key="1">
            <Link to={`/${currentUserName}?tab=articles`}>{<Book size="16" />}&nbsp;Переглянути всі</Link>
         </Menu.Item>
         {/*<Menu.Item key="2">
            <Link to={`/bookmarks`}>{<Bookmark size="16" />}&nbsp;Закладки</Link>
         </Menu.Item>*/}
         <Menu.Item key="3">
            <Link to={`/settings`}>
               {<Badge dot={!isVerify} offset={[-1, 1]}><Settings size="16" /></Badge>}
               &nbsp;Налаштування
            </Link>
      	</Menu.Item>
         <Menu.Divider />
         <Menu.Item key="4">
            <Link to="#" onClick={logOut}><LogOut size="16" />&nbsp;Вийти</Link>
         </Menu.Item>
      </Menu>
   );
            ...
            <div className="o-container o-wrapper">
               <Row gutter={30}>
                  <Col span="24">
                     ...
                     <Col className="header-buttons-hide">
                        <Space size="middle">
                           <Link to={`/${currentUserName}`}><Button>{currentUserName}</Button></Link>
                           <Dropdown overlay={dropdownMenu} trigger={['click']}>
                              <span className="c-dropdown-toggle">
                                 <Badge dot={!isVerify}>
                                    <img id="dropdownUserAvatar" alt={ `@${currentUserName}` } className="c-avatar_dropdown" src={currentUserAvatar} />
                                 </Badge>
                              </span>
                           </Dropdown>
                           <AutoComplete
...
export default HeaderIsRegister;
